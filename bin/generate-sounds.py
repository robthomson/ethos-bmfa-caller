#!/usr/bin/env python3
"""
Generate Ethos-compatible WAV files for bmfa-caller from soundlist CSVs.

Scans src/bmfa-caller/categories/ for all soundlist.csv files, then calls
...
Usage:
    ...
    python generate-sounds.py --voice en-GB-Neural2-A --speed 0.9
    python generate-sounds.py --csv categories/airplane/airplane-b/soundlist.csv
"""
...
DEFAULT_ROOT  = os.path.normpath(os.path.join(SCRIPT_DIR, "..", "src", "bmfa-caller", "categories"))
...
def main():
    parser = argparse.ArgumentParser(description="Generate BMFA Caller sound files via Google TTS")
    parser.add_argument("--sounds-dir",   default=DEFAULT_ROOT,
                        help=f"Root categories directory (default: {DEFAULT_ROOT})")
